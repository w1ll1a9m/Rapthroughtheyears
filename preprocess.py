...
def tokenize(text, wordCountBefore, textID):
    totalAdjectives = 0
    totalAdverbs = 0
    totalVerbs = 0
...
### NO POS TAGGING BEGIN (If you don't want to use POS Tagging keep this section uncommented) ###
    ...
    for w in tokens:
        ...
        final_word = replaceElongated(w) # Technique 11: replaces an elongated word with its basic form, unless the word exists in the lexicon
        if len(final_word)>1:
...
### POS TAGGING END ###
    ...
    with open("result.txt", "a") as result:
        result.write(textID+","+","+onlyOneSentence)
        
    return finalTokens


f = open("hip_hop_nocontracted_v4_lowercase.csv","r", encoding="utf8", errors='replace').read()
...
for line in f.split('XX'):
    ...
    tokens = tokenize(text, wordCountBefore, textID)  
